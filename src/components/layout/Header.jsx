...
const Header = ({ onOpenQuote }) => {
  ...
  const getLinkClass = (isActive) =>
    `text-[11px] font-black uppercase tracking-[0.2em] transition-all px-4 py-2 rounded-md ${
      isActive ? "bg-[#0ea5e9] text-white" : "text-slate-600 hover:text-[#000]"
    }`;
    ...
    >
      <div className="container mx-auto px-6 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-3 md:gap-4 group">
          <div className="h-12 w-12 md:h-16 md:w-16 rounded-full overflow-hidden border-2 border-slate-100 shadow-sm transition-transform group-hover:scale-105 flex-shrink-0">
            <img
              src="/logo.jpeg"
              alt="NativeShine"
          ...
          <div className="flex flex-col md:flex-row md:items-center border-l-2 border-slate-100 pl-3 md:pl-4 py-1">
            <span className="text-lg md:text-2xl font-black text-slate-900 tracking-tighter uppercase leading-none">
                ...
                <NavLink
                  ...
                  className={({ isActive }) => {
                    const isHashMatch = link.hash
                      ...
                      : !location.hash;
                    const trulyActive = isActive && isHashMatch;
                    return `text-sm font-black uppercase tracking-widest p-4 rounded-md transition-all ${
                      trulyActive
